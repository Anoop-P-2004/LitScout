...
if __name__ == "__main__":
    initial_state = LitScoutState(
        user_prompt="Conduct a literature survey on the use of transformer-based models for sentiment analysis in low-resource languages, focusing only on  journal articles and conference papers published in the last 7 years on IEEE, ACM, ScienceDirect,Elsevier."
    )
    
    print("="*70)
    print("STARTING LITSCOUT ORCHESTRATOR")
    print("="*70)
    ...
    try:
        ...
        if final_state and hasattr(final_state, 'error_messages') and final_state.error_messages:
            ...
            for error in final_state.error_messages[-3:]:
                print(f"  • {error}")
